package com.financial_system.financial_system.income.controller;
...
public class IncomeController {
    ...
    public Income selectOne(String id) {
        return this.incomeServiceImpl.queryById(id);
    }
    /**查询管理员 API0015
     * 查询多条数据
     *
    ...
    PageResult querybyPage(@RequestBody PageParam vo, HttpServletRequest request)throws  Exception{
        ...
        try{
            ...
            List<Income> data=incomeServiceImpl.queryByPage(vo);
    ...
    @ResponseBody
    ResponseDto insert(@RequestBody Income Consumption, HttpServletRequest request){
        ResponseDto result=null;
        try{
            String token = request.getHeader("X-Token");
            User user=userServiceImpl.tokenGetUserinfo(token);
            Consumption.setUserid(user.getId());
            incomeServiceImpl.insert(Consumption);
    ...
    @ResponseBody
    ResponseDto update(@RequestBody Income Consumption, HttpServletRequest request){
        ResponseDto result=null;
        try{
            String token = request.getHeader("X-Token");
            User user=userServiceImpl.tokenGetUserinfo(token);
            Consumption.setUserid(user.getId());
            incomeServiceImpl.update(Consumption);
            result=new ResponseDto(200,"success",null);
        }catch (Exception e){
            result=new ResponseDto(500,"error:"+e.getMessage(),null);
            e.printStackTrace();
    ...
    ResponseDto deleteById(@RequestBody String id){
        ResponseDto result=null;
        try{
            incomeServiceImpl.deleteById(id);
            result=new ResponseDto(200,"success",null);
        }catch (Exception e){
            result=new ResponseDto(500,"error:"+e.getMessage(),null);
            e.printStackTrace();
